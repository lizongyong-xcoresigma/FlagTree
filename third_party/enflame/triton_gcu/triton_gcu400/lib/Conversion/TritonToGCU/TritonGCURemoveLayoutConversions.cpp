...
namespace mlir {
#define GEN_PASS_DEF_TRITONGCUREMOVELAYOUTCONVERSIONSPASS
#include "Conversion/Passes.h.inc"
} // namespace mlir

#define DEBUG_TYPE "triton-gcu-remove-layout-conversions"
...
public:
  ...
  struct LayoutInfo {
    explicit LayoutInfo(Attribute encoding) { encodings.insert(encoding); }
    LayoutInfo() {}
    llvm::SmallSetVector<Attribute, 8> encodings;
  };
  explicit LayoutPropagation(FuncOp F) : funcOp(F) {}
...
public:
  explicit LayoutRematerialization(FuncOp F) : funcOp(F) {}
  ...
  // TODO(Official): Merge the three hoistConvert*(); functions as they are
  // duplicate code
...
bool isLayoutAnchor(Operation *op) {
  ...
  if (isa<DotOp, DotScaledOp, triton::gcu::WarpGroupDotOp, AtomicRMWOp,
          AtomicCASOp>(op))
    return true;
...
void LayoutPropagation::resolveConflicts() {
  /// Added for GCU
  /// ------------------------------ start point -------------------------------
  // Pre-compute frequency of each BlockedEncodingAttr across all values.
  // Used to break ties when multiple BlockedEncodingAttr candidates conflict.
  DenseMap<Attribute, int> blockedFreq;
  for (auto &entry : layouts) {
    for (Attribute e : entry.second.encodings) {
      if (isa<BlockedEncodingAttr>(e))
        blockedFreq[e] = blockedFreq.lookup(e) + 1;
    }
  }
  /// ------------------------------- end point --------------------------------
  ...
  for (auto &it : layouts) {
    ...
    // TODO(Official): add a proper heuristic.
    ...
    /// Added for GCU
    /// ----------------------------- start point ------------------------------
    // When all candidates are BlockedEncodingAttr (no MmaEncodingTrait
    // matched), prefer the encoding that appears most frequently across the
    // function. This avoids arbitrary selection that can result in unnecessary
    // convert_layout pairs (e.g., #blocked → #blocked2 → compute → #blocked).
    if (!isa<MmaEncodingTrait>(encoding)) {
      bool allBlocked = llvm::all_of(info.encodings, [](Attribute e) {
        return isa<BlockedEncodingAttr>(e);
      });
      if (allBlocked) {
        int maxFreq = 0;
        for (Attribute e : info.encodings) {
          int f = blockedFreq.lookup(e);
          if (f > maxFreq) {
            maxFreq = f;
            encoding = e;
          }
...
__attribute__((unused)) void LayoutPropagation::dump() {
  for (auto it : layouts) {
    llvm::errs() << "Value: ";
...
Value LayoutPropagation::getValueAs(Value value, Attribute encoding) {
  if (auto tensorType = dyn_cast<RankedTensorType>(value.getType())) {
    ...
    Value converted = ConvertLayoutOp::create(rewriter, value.getLoc(), tmpType,
                                              rewrittenValue);
    // TODO(Official): we could cache the conversion.
    return converted;
  }
  return value;
}
...
Operation *LayoutPropagation::rewriteWhileOp(scf::WhileOp whileOp) {
  ...
  for (unsigned i = 0; i < whileOp.getNumRegions(); ++i) {
...
#if TRITON_VERSION >= 36
           isa<BroadcastOp, Fp4ToFpOp, ConvertLayoutOp, UpcastFpOpInterface>(
               op) ||
#else
           // Triton 3.5 无 UpcastFpOpInterface（Triton 3.6 引入）
           isa<BroadcastOp, Fp4ToFpOp, ConvertLayoutOp>(op) ||
#endif
           isView(op);
  };
...
void LayoutRematerialization::hoistConvertIntoConditionals(
  ...
  // Rematerialize failed hoists right before the conditional, and hoist those
...
struct TritonGCURemoveLayoutConversionsPass
    : public mlir::impl::TritonGCURemoveLayoutConversionsPassBase<
          TritonGCURemoveLayoutConversionsPass> {
public:
  // Cleanup convert ops.
